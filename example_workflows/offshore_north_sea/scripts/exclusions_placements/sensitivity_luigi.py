...
class SensitivityAnalysis(luigi.Task):
    ...
    def output(self):
        ...
        """
        return luigi.LocalTarget(os.path.join(ConfigLoader().get_path("output"), 'logs', 'copmleted_tasks', 'SensitivityAnalysis_complete.txt'))
    ...
    def run(self):
        ...
        result = subprocess.run(['/usr/bin/bash', './scripts/exclusions_placements/sensitivity_wrapper.sh'], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
